...
def postprocess(frame, outs, posNP):
    ...
    for (objectID, centroid, ) in objects.items():
        ...
        if len(posNP) > 3:
            polygon = Polygon(posNP[0:4])
            if polygon.contains(point):  
                cv.circle(frame, (centroid[0], centroid[1]), 8, (0, 0, 255), -1)
...
while cv.waitKey(1) < 0:
    ...
    #get mouse click x, y coordinates for four clicks and print to terminal
    if len(posList) <= 4:
